...
export const GnuDbModal: React.FC<GnuDbModalProps> = ({ isOpen, onClose, onSuccess, t, albumTitle, performer }) => {
    ...
    React.useEffect(() => {
        ...
        (window as any).ipcRenderer.on('gnudb:set-cdid', (_: any, id: string) => {
            setGnucdid(id);
        });

        return () => {
            (window as any).ipcRenderer.removeAllListeners('gnudb:set-cdid');
        };
    }, [isOpen]);
        ...
        try {
            ...
            if (response?.result) {
                ...
                if (err.includes('404') || err.includes('invalid xmcd')) {
                    setError(`${t.invalidXmcd}<br>wrong gnucdid?`);
                } else {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm px-4">
            ...
            >
                ...
                {/* Intelligent Link (Direct to search for GnuDb) */}
                ...
                <div>
                    <div className="border border-brand-input-border rounded-full px-3 py-1 flex items-center bg-transparent focus-within:border-brand-orange transition-colors">
                        <input
                            ...
                            placeholder={t.gnucdidPlaceholder}
                        />
                    </div>
                </div>
                ...
                {error && (
                    <div className="flex items-start gap-3 animate-in fade-in slide-in-from-left-2 duration-300">
                        ...
                        <div className="flex gap-2.5 items-start">
                            ...
                            <span className="text-brand-orange text-modal-small leading-tight font-light whitespace-pre-wrap" dangerouslySetInnerHTML={{ __html: error }} />
                        </div>
                    </div>
                )}
